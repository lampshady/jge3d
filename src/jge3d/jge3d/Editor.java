...
public class Editor {
	...
	public void renderCurrentBlock() {
		GL11.glPushMatrix();
			editor_origin.x = (float)Math.floor((double)editor_origin.x);
			editor_origin.y = (float)Math.floor((double)editor_origin.y);
			
			//Z needs to be replaced with -layer * cube_size
			editor_origin.z = (float)Math.floor((double)editor_origin.z);
